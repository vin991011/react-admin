...
import './index.less';
const Logo = require('@/assets/img/喜多.png');
const { Sider } = Layout;
  ...
  return routes.map((v) => {
    ...
    if (!hideMenu) {
      ...
      return getItem(
        ...
        key,
        icon,
        hasChildren ? mapMenuItem(children) : ''
      );
    }
  });
}
export default function MySider(props) {
  const [defaultOpenKeys, setDefaultOpenKeys] = useState([
    getSession('curOpenKeys') || 'system',
  ]);
  const [defaultSelectedKeys, setDefaultSelectedKeys] = useState([
    getSession('curSelectedKeys') || '/',
  ]);
  ...
  return (
    <Sider
      ...
      <Menu
        ...
        mode="inline"
        items={mapMenuItem(routes[0].children)}
